...
public class NSGAIII<S extends Solution<?>> extends AbstractGeneticAlgorithm<S, List<S>> {
  ...
  protected BatchSolutionListEvaluator<S> evaluator ;
  protected OffSpringEvaluator<S> offSpringEvaluator ;
  ...
  public NSGAIII(NSGAIIIBuilder<S> builder) { // can be created from the NSGAIIIBuilder within the same package
    ...
    offSpringEvaluator  = builder.getOffSpringEvaluator();
    /// NSGAIII
  ...
  @Override
  protected List<S> evaluateOffSpringPopulation(List<S> population, List<S> offspringPopulation) {
  	population = offSpringEvaluator.evaluate(population, offspringPopulation, getProblem());
  	return population;
  }
  @Override
